...
void solution2(string dictionary_name, string out_name, char params_char, int params_num, string params_string) {
	...
	for (int i = 0; i < N; i++) {
		n = A[i].find_first_of(':');
		if (A[i][n + 1] == params_char || A[i][n + 1] == params_char - 32) {
			fout << A[i] << endl;
		}
	}
	fout.close();
